...
SELECT * FROM Movie_spoken_languages;
...
BEGIN
 ...
 cursorLoop: LOOP
  ...
  SET @sql_text = CONCAT('INSERT IGNORE INTO Crew VALUES (', idMovie, ', ', REPLACE(idJSON,'\'',''), ', ', REPLACE(jobJSON,'\'',''), ', ', REPLACE(departmentJSON,'\'',''), ', ', REPLACE(credit_idJSON,'\'',''), '); ');
	PREPARE stmt FROM @sql_text;
	EXECUTE stmt;
	DEALLOCATE PREPARE stmt;
